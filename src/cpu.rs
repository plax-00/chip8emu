...
impl CPU {
	pub fn new() -> Self {
		Self { v: [0; 16], i: 0, dt: 0, st: 0, pc: PROGRAM_START, sp: 0, stack: [0; 16] }
	}
	...
	pub fn execute(&mut self, instruction: &Instruction, display: &mut Display, memory: &mut Memory, keys: &[bool; 16]) {
		...
		match *instruction {
			...
			Ins::SE_byte(vx, byte) => if self.read_reg(vx) == byte { self.pc += 2 },
			Ins::SNE_byte(vx, byte) => if self.read_reg(vx) != byte { self.pc += 2 },
			Ins::SE_reg(vx, vy) => if self.read_reg(vx) == self.read_reg(vy) { self.pc += 2 },
			...
			Ins::SNE_reg(vx, vy) => if self.read_reg(vx) != self.read_reg(vy) { self.pc += 2 },
